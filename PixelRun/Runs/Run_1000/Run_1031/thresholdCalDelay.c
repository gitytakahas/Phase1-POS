...
//   create a root file with an histogram and an ntuple.
  ...
  insummary.open("VcThrCalDelaySummary_1.txt");
  ...
  while (insummary.good()){
    ...
    eff->SetMaximum(1.0);
    eff->Draw("colz");
    std::stringstream test; test.str("");
    test << "hist_" << roccounter << ".root";
    eff->SaveAs(test.str().c_str());

    deltaCalDelay=0.05*(max1-min1);
    deltaVcThr=0.05*(max2-min2);

    l=new TLine(oldcalDelay-deltaCalDelay,oldthreshold,
